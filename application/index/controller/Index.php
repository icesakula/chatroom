...
class Index extends Controller
{
    ...
    public function getList()
    {
        ...
        if(!empty($friend_id_arr)){
            $other = db('chatuser')->where('id','in',$friend_id_arr)->select();
        }else{
            $other = [];
        }
        ...
        //查询当前用户的所处的群组
        $groupArr = [];
        $groups = db('groupdetail')->field('groupid')->where('userid', session('uid'))->group('groupid')->select();
        if( !empty( $groups ) ){
